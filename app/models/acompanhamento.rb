class Acompanhamento < ApplicationRecord
  ...
  def render_info_para_profissional
    ...
    return "#{acompanhamento_tipo.tipo.to_s.upcase} - #{p.nome_completo} #{p.render_sexo_sigla} #{if r then '(respondido por ' + r.nome_completo + ')' end} (#{atendimento.count} #{atendimento.count == 1 ? 'sessão' : 'sessões'})"
  end
